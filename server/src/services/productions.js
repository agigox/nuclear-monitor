...
export const getProductionsPerUnit = async (input, { rteToken }) => {
  ...
  const productions = response.actual_generations_per_unit.map(
    ({ start_date, end_date, unit, values }) => ({
      startDate: start_date,
      endDate: end_date,
  ...
  return {
    length: productionsGroupedByCategoryAndEicCode.length,
    items: productionsGroupedByCategoryAndEicCode,
  };
};
